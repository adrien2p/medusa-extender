...
class MedusaEventEmitter extends EventEmitter {
	 ...
	 */
	public register<T>(eventName: string | symbol, propertyName: string, metatype: Metatype<T>): void {
	...
	public registerListeners(container: AwilixContainer): void {
		...
		for (const listenerDescriptor of this.#listeners.values()) {
			...
			const metatypeName = (metatype as any).name ?? metatype;
			...
			const metatypeInstance =
				((container as any).has(`craftshop/${formattedMetatypeName}`) &&
					container.resolve(`craftshop/${formattedMetatypeName}`)) ||
