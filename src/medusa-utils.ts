...
import { Constructor, MedusaEntity, MedusaEntityStatic, MedusaRepository, MedusaRepositoryStatic } from './types';
...
export class Utils {
     ...
     * @param baseCtors
     */
    static repositoryMixin<TEntity,
        TOverriddenType,
        TRepositoryStatic extends Constructor<MedusaRepository<MedusaRepositoryStatic<TOverriddenType>>> = Constructor<MedusaRepository<MedusaRepositoryStatic<TOverriddenType>>>,
        TMedusaRepositoryStatic extends ObjectType<Repository<TOverriddenType>> = ObjectType<Repository<TOverriddenType>>>
    (
        derivedCtor: TRepositoryStatic,
        ...baseCtors: TMedusaRepositoryStatic[]
    ): MedusaRepositoryStatic<keyof TMedusaRepositoryStatic & keyof TRepositoryStatic>  {
        baseCtors.forEach((baseCtor) => {
            // Instance members
            Object.getOwnPropertyNames((baseCtor as any).prototype).forEach((name) => {
                if (name !== 'constructor') {
                    (derivedCtor as any).prototype[name] = (baseCtor as any).prototype[name];
                }
            });
        });
        return derivedCtor;
